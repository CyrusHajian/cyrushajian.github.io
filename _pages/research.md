---
title: "Research"
layout: single
permalink: /research/
author_profile: true
page_js:
  - /assets/js/research-sections.js
---

## Research Areas

<div class="research-page">
  <div class="research-grid" data-research-grid>
    <button class="research-card" type="button" data-research-target="detail-1">
      Placeholder 1
    </button>
    <button class="research-card" type="button" data-research-target="detail-2">
      Placeholder 2
    </button>
    <button class="research-card" type="button" data-research-target="detail-3">
      Placeholder 3
    </button>
    <button class="research-card" type="button" data-research-target="detail-4">
      Placeholder 4
    </button>
  </div>

  <section class="research-detail" id="detail-1" hidden>
    <button class="research-back" type="button" data-research-back>
      ← Back
    </button>
    <h3>Placeholder 1</h3>
    <p>Details for this area will go here.</p>
  </section>

  <section class="research-detail" id="detail-2" hidden>
    <button class="research-back" type="button" data-research-back>
      ← Back
    </button>
    <h3>Placeholder 2</h3>
    <p>Details for this area will go here.</p>
  </section>

  <section class="research-detail" id="detail-3" hidden>
    <button class="research-back" type="button" data-research-back>
      ← Back
    </button>
    <h3>Placeholder 3</h3>
    <p>Details for this area will go here.</p>
  </section>

  <section class="research-detail" id="detail-4" hidden>
    <button class="research-back" type="button" data-research-back>
      ← Back
    </button>
    <h3>Placeholder 4</h3>
    <p>Details for this area will go here.</p>
  </section>
</div>
